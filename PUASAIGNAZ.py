...
class instagram:
	...
	def methodku(self):
		urut = []
		urut.append(Panel(f"API NERAKA{H2}Recom{P2}",title=f"{R2}01{P2}",width=25,padding=(0,4),style=f"#FFFFFF"))
...
def meng(self):
		menudump.append('mengikuti')
		prints(Panel(f"{P2}pastikan username publik jangan private",width=80,style=f"#FFFFFF"))
		nama=input(f'{P}[{B}?{P}] masukan username : {H}')
		id=self.idAPI(self.cookie,nama)
		info=self.ifoAPI(self.cookie,'https://i.instagram.com/api/v1/friendships/%s/following/?count=100000',id)
		self.passwordAPI(info)
